...
// Constants -------------------------------------------------------------
  ...
  // Fields ----------------------------------------------------------------
  private final Vector retainAttrs = new Vector();  // List of attributes to retain
  ...
  public static Enumeration getNameEnum( String name ) {
    ...
    while (!name.equals("")) {
      ...
      if (posP == -1 && posC > 0) {
        ...
        while ((posC + 1 < name.length()) && (name.charAt(posC + 1) == CLASS_LEVEL)) {
          posC++;
        }
        name = name.substring(posC + 1);
      }
      if (posP != -1 && posC == -1) {
        cons = new Cons(new Character(PACKAGE_LEVEL), name.substring(0, posP));
        name = name.substring(posP + 1);
      }
      if (posP != -1 && posC != -1) {
        if (posP < posC) {
          cons = new Cons(new Character(PACKAGE_LEVEL), name.substring(0, posP));
  ...
  private Cl findClass( PkCl parent, String pName ) {
    for (Enumeration enumeration = parent.getClassEnum(); enumeration.hasMoreElements(); ) {
  ...
  private boolean modifierMatch( int level, int mods ) {
    if (level == YGuardRule.LEVEL_NONE) {
      return false;
    }
  ...
  public void retainClass( String name, int classLevel, int methodLevel, int fieldLevel, boolean retainHierarchy ) {
    ...
    for (Enumeration clEnum = getClEnum(name, classLevel); clEnum.hasMoreElements(); ) {
      ...
      if (retainHierarchy && classLevel != YGuardRule.LEVEL_NONE) {
        retainHierarchy(classItem);
      }
    ...
    while (i < signature.length()) {
      ...
      switch (ch) {
        ...
        case 'T': {   // Template name
          ...
          newSignature.append(signature, i, pos);
          i = pos;
          break;
        }
        case '<': {
