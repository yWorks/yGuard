...
// Class Methods ---------------------------------------------------------
  ...
  public int getModifiers() {
    int mods = 0;
    if ((u2innerClassAccessFlags & 0x0001) == 0x0001) {
      mods |= Modifier.PUBLIC;
    }
    if ((u2innerClassAccessFlags & 0x0002) == 0x0002) {
      mods |= Modifier.PRIVATE;
    }
    if ((u2innerClassAccessFlags & 0x0004) == 0x0004) {
      mods |= Modifier.PROTECTED;
    }
    if ((u2innerClassAccessFlags & 0x0008) == 0x0008) {
      mods |= Modifier.STATIC;
    }
    if ((u2innerClassAccessFlags & 0x0010) == 0x0010) {
      mods |= Modifier.FINAL;
    }
    if ((u2innerClassAccessFlags & 0x0200) == 0x0200) {
      mods |= Modifier.INTERFACE;
    }
    if ((u2innerClassAccessFlags & 0x0400) == 0x0400) {
